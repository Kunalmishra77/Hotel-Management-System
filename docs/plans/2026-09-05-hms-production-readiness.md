...
- ✅ **Phase 8** (2026-09-05) — **Data Entry** page (`/data-entry`, Super-Admin): capture a historical record's photo (stored; filed as an encrypted ID scan when it's an ID), optional **AI-assist** that proposes guest fields from pasted record text (empty with the mock provider → manual; a configured provider fills it), review, and save via the audited `createGuest`. New `guest-extract` AI feature. Build green, pushed. *Full photo-OCR of bills/bookings activates when a vision AI provider is enabled (a client integration, like the others).*
- ✅ **Phase 9** (2026-09-05) — A-Z portal/module review delivered as a prioritised backlog: `docs/plans/2026-09-05-hms-phase9-review.md`. Covers every portal + cross-cutting + tech, P1/P2/P3, with a suggested next sequence. This is the ongoing refinement backlog (each item = a small build→test→review phase).
