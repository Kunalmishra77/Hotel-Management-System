...
- **AC-20 (coupon):** Given a valid coupon `SAVE10` entered at checkout, when previewing, then `06.validateCoupon` reduces the GST-inclusive total + deposit and **nothing is consumed**; on payment-success confirm, `06.applyCoupon({reservationId})` redeems it **once** in the confirmation transaction (`CouponRedeemed`); an abandoned/failed checkout redeems nothing. An invalid/expired/exhausted coupon shows the reason and the guest proceeds without it — charged total always equals the shown total. (FR-23/24)
