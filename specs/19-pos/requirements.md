...
- **FR-4 (ubiquitous):** Compute F&B GST **per line** by HSN/SAC and applicable rate using the single shared split (`lib/tax`, the same one 06 uses) — intra-state → CGST+SGST split equally, inter-state → IGST — decided by property state vs place of supply (`business-rules.md` §10). Rate is **config-driven** per outlet/menu-item, never hard-coded (`business-rules.md` §11).
