...
- **FR-5 (ubiquitous):** Record source, adults, children, check-in/out dates; compute `nights` in property-local days (min 1). Client §2's "expected arrival" need is satisfied by the booked `checkInDate`/`checkOutDate` (`@db.Date`) plus the actual `checkInAt`/`checkOutAt` timestamps stamped at check-in/out — no separate expected-arrival-time field is kept.
