...
## Form C / FRRO — foreign-guest arrival reporting (addendum 2026-08-09)
India requires every hotel to report a foreign national's arrival to the FRRO/FRO (Form C). Binding client rule (MoM 2026-08-03): **the C-Form is captured when a foreign guest's passport is on file** (i.e. a PASSPORT/VISA id is uploaded), and staff must be able to **track which C-Forms have been reported to the FRRO**.

- **FR-24 (capture — already built, T7):** When a reservation's guest has a PASSPORT or VISA id on file, the check-in flow requires a Form C step. `saveCForm` extracts passport fields via the AI layer (proposes only; reception verifies), persists a **masked** snapshot (full passport/visa numbers stay encrypted on `GuestId`, never on `CForm`), renders the Form C PDF to **encrypted object storage** (DB keeps only `pdfObjectKey`), and emits `CFormGenerated` + audit. The C-Form starts `status = GENERATED`.
- **FR-25 (register):** Provide a property-scoped **Form C register** listing every foreign-guest C-Form — guest name, nationality, arrival/departure, status, and submission reference — with cursor pagination. Read requires `reservation:view`; it exposes **no** passport/visa numbers (only nationality). The generated PDF is re-downloadable from the register through the same authorized, non-public path as payslips (`getCFormBytes`).
- **FR-26 (submission tracking):** `markCFormSubmitted(cformId, submissionRef)` records that a C-Form was submitted on the e-FRRO portal: `GENERATED → SUBMITTED`, stores the portal `submissionRef`, emits `CFormSubmitted` + audit. Requires `checkin:perform`. Idempotent — re-recording with the same ref is a no-op; the action never regenerates or mutates the PDF. **Honest live blocker (`integrations.md`):** we do not auto-submit to the Bureau of Immigration e-FRRO portal (that needs certified access); staff submit on the portal and record the reference here. An e-FRRO submit adapter can be added later behind an interface without touching call sites.
