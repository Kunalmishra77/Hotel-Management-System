/**
 * ISO 3166 countries with international dial codes — for guest onboarding
 * (country of residence + mobile country code). Sorted A→Z by name. Dial codes
 * are the primary country calling code (a few territories share a parent code).
 */
...
/** Distinct dial codes, ordered for a compact country-code picker (India first). */
export const DIAL_CODES: { dial: string; labels: string[] }[] = (() => {
  const byDial = new Map<string, string[]>();
  for (const c of COUNTRIES) {
    const list = byDial.get(c.dial) ?? [];
    list.push(c.iso2);
    byDial.set(c.dial, list);
  }
  return [...byDial.entries()]
    .map(([dial, labels]) => ({ dial, labels }))
    .sort((a, b) => {
      if (a.dial === "+91") return -1;
      if (b.dial === "+91") return 1;
      return Number(a.dial.slice(1)) - Number(b.dial.slice(1));
    });
})();
